...
export class InitialTables1660045798480 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TABLE \`user\` (\`id\` varchar(36) NOT NULL, \`username\` varchar(100) NOT NULL, \`email\` varchar(320) NOT NULL, \`phone\` varchar(100) NOT NULL, \`refresh_token\` varchar(1000) NULL, \`password_hash\` varchar(300) NOT NULL, \`password_salt\` varchar(128) NOT NULL, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), UNIQUE INDEX \`IDX_78a916df40e02a9deb1c4b75ed\` (\`username\`), UNIQUE INDEX \`IDX_e12875dfb3b1d92d7d7c5377e2\` (\`email\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
      `CREATE TABLE \`booking\` (\`id\` varchar(36) NOT NULL, \`date\` date NOT NULL, \`time\` time NOT NULL, \`length_of_service_in_minutes\` smallint NOT NULL, \`contact_method\` enum ('EMAIL', 'PHONE') NOT NULL DEFAULT 'EMAIL', \`email\` varchar(320) NOT NULL, \`phone\` varchar(100) NULL, \`comment\` varchar(200) NULL, \`first_name\` varchar(100) NOT NULL, \`last_name\` varchar(100) NOT NULL, \`status\` enum ('NEW', 'APPROVED', 'CANCELED') NOT NULL DEFAULT 'NEW', \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deleted_at\` datetime(6) NULL, \`serviceId\` varchar(36) NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
